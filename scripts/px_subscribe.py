...
async def default_subscription_loop(config, secret, ws_url, topic, pubsub_node_name):
    '''
...
async def session_dedup_loop(config, secret, ws_url, topic, pubsub_node_name):
    '''
...
def run(external_loop=None):
    ...
    if not config.subscribe_all:
        ...
        if external_loop:
            loop = external_loop
            main_task = asyncio.run_coroutine_threadsafe(subscription_loop(config, secret, ws_url, topic, pubsub_node_name), loop)
        else:
